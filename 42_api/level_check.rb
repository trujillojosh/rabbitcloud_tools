...
def fremont_users
	...
	res = Array.new
	i = 109
	while (tmp = TOKEN.get('/v2/campus/7/users', params: {page: {number: i}}).parsed).length > 0
		puts 'i is ' + i.to_s
		tmp.each do |page|
			res.push(page)
		end
...
def user_test(users, level)
	...
	users.each do |person|
		...
		if response['cursus_users'][0]['level'] >= level
			puts 'success, ' + person['login'] + 'is ' + response['cursus_users'][0]['level'].to_s
			res.push(person)
		end
		test_i += 1
	end
...
# puts user_test(fremont_users).to_i
puts write_logins(user_test(fremont_users, 7))